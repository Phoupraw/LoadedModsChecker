...
public interface FabricUtils {
    ...
    Codec<List<Triple<FluidVariant, Long, Integer>>> SLOTTED_FLUID_STORAGE_CODEC = new MapTripleCodec<>(
      FluidVariant.CODEC,
      Codec.LONG.fieldOf("amount").codec(),
      Codec.INT.fieldOf("slot").codec()
    ).listOf();/*.xmap(pair->new ResourceAmount<>(pair.getFirst(),pair.getSecond()),snapshot-> Pair.of(snapshot.resource())))*/
    //static <L, M, R> Codec<Triple<L, M, R>> tripleMap(Codec<L> left, Codec<M> middle, Codec<R> right) {
    //    return new MapTripleCodec<>(left, middle, right);
    //}
